...
def save_samples(index, latent_tensors, show=True):
    ...
    for j in range(fake_no):
        ...
        for real_images in tqdm(train_dl):
            cos_sim,dist=get_eud_and_cossim(real_images,fake_images[0][0].cpu().detach().numpy())
            #cos_sim_per_epoch = cos_sim_per_epoch+cos_sim
            #dist_per_epoch = dist_per_epoch+dist

        fake_fname = f'1-generated-csv-{str(index).zfill(5)}-{str(j).zfill(5)}.csv'
...
if __name__ == '__main__':
    ...
    DATA_DIR = 'C:/Users/Nirho/Desktop/gasf/gasf-csv/bins/gasf_csv_bin4_train/Netflix/vid2'
    ...
    generator.load_state_dict(torch.load('Gvid2.pth',map_location='cuda:0'))
    discriminator.load_state_dict(torch.load('Dvid2.pth',map_location='cuda:0'))
    ...
    fixed_latent = torch.zeros(1, latent_size, 1, 1, device=device)
    #fixed_latent[0][0]=1
    #print(fixed_latent)

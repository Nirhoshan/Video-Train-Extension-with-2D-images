...
def save_samples(index, latent_tensors, show=True):
    ...
    for j in range(fake_no):
        ...
        for real_images in tqdm(train_dl):
            cos_sim, dist, ssim_score, mse = get_eud_and_cossim(real_images, fake_images[0][0].cpu().detach().numpy())
        ...
        # if index==200:
        fake_fname = f'0-generated-csv-{str(index).zfill(5)}-{str(j).zfill(5)}.csv'
...
def get_eud_and_cossim(real,fake):
    ...
    dist=dist/len(real)
    mse=mse/len(real)
    return cos_sim,dist,ssim_score,mse
...
def fit(epochs, lr, start_idx=1):
    ...
    for epoch in range(epochs):
        ...
        if epoch%50==0:
            torch.save(generator.state_dict(), 'weights20/G20yt' + str(epoch) +'-gm.pth')
            torch.save(discriminator.state_dict(), 'weights20/D20yt' + str(epoch) +'-gm.pth')
...
if __name__ == '__main__':
    ...
    DATA_DIR = 'gasf_20_pretrain_Youtube'
    ...
    #generator.load_state_dict(torch.load('G0.pth'))
    #discriminator.load_state_dict(torch.load('D0.pth'))
    ...
    torch.save(generator.state_dict(), 'weights20/G20yt600-gm.pth')
    torch.save(discriminator.state_dict(), 'weights20/D20yt600-gm.pth')

    from IPython.display import Image

    Image('./generated1/1generated-images-0001.png')
  ...
  #  plt.title('Scores');
    ...
    plt.savefig("cosine_similarity_0.png")
    plt.clf()
    plt.plot(eud)
    plt.savefig("euclidean_distance_0.png")
    plt.clf()
    plt.plot(ssim_scores)
    plt.savefig("ssim_score_0.png")
    plt.clf()
    plt.plot(mses)
    plt.savefig("mses_0.png")
    plt.show()
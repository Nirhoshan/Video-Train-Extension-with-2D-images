...
def run_vid_clf(x_train, y_train, x_test, y_test, platform, synth_size, random_state, bin_size,train_data_size):
    ...
    print(model)
    model.fit(x_train, y_train, epochs=240, batch_size=8, verbose=True)
    ...
    path_to_save = 'dl_clf/results/diff_bin_size_analysis_with_synth_data_reduced_act_traces/mtf/' + '/' + 'split_the_traces_in_a_video' + '/' + \
...
def main():
    ...
    for bin in bins:
        ...
        #feat_data_synth, gt_train_synth = load_synth_data(im_type=im_type, platform=platform, d_type='synthesized',bin_size=bin)
        ...
        for p in range(3):
            ...
            for r in range(5):
                ...
                # store the data
                columns = list(np.arange(1).astype(str))

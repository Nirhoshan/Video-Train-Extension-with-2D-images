...
def load_data_actual(im_type, platform, d_type, bin_size):
    if GPU:
        ...
        path_in = 'C:/Users/Nirho/Desktop/gasf/gasf-csv/for_clf2'
    print(path_in)
    all_platforms = []
...
def run_vid_clf(x_train, y_train, x_test, y_test, platform, synth_size, random_state, bin_size,train_data_size,test_acc,epoch):
    print('Create model')
    # example of a 3-block vgg style architecture
    y_train = to_categorical(y_train)
    ...
    model.fit(x_train, y_train, epochs=epoch, batch_size=8, verbose=True)
    ...
    path_to_save = 'dl_clf_epoch_analysis/results/diff_bin_size_analysis_with_synth_data_reduced_act_traces/gasf/' + '/' + 'split_the_traces_in_a_video' + '/' + \
        ...
        for p in range(3):
            ...
            for r in range(1):
                ...
                for synth_size in range(0, 301, 30):
                    ...
                    epoch_max = 120
                    test_acc = []
                    for epoch in range(0, epoch_max + 1, 10):
                    ...
                    plt.clf()
                    plt.plot(test_acc)
                    plt.savefig('dl_clf_epoch_analysis/results/diff_bin_size_analysis_with_synth_data_reduced_act_traces/gasf/' + '/' + 'split_the_traces_in_a_video' + '/' + \
                               platforms[platform] + '/bin_' + str(bin) + '/train_data_size_'+str(train_data_size)+'/Random_set_' + str(r)+'_test_acc_'+str(synth_size)+'.png')
                    print(test_acc)
                # store the data
